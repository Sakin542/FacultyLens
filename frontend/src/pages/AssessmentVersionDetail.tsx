...
const field = 'w-full rounded-lg border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#111111] px-3 py-2 text-sm text-[#111111] dark:text-white';
        ...
        <>
          ...
          {v.validation && v.validation.status !== 'VALID' && (
            <Card data-testid="version-validation" className={`p-4 space-y-1 ${v.validation.status === 'INVALID' ? 'border-red-200' : ''}`}>
              <h3 className="text-sm font-semibold text-[#111111] dark:text-white">Validation: {humanize(v.validation.status)}</h3>
          ...
          {editing ? (
            <form noValidate onSubmit={(e) => { e.preventDefault(); save(); }} className="space-y-4" data-testid="version-edit-form">
              <Card className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
                <label className="text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Title<input aria-label="Version title" value={meta.title ?? ''} onChange={(e) => setMeta({ ...meta, title: e.target.value })} className={`${field} mt-1 font-normal normal-case tracking-normal`} /></label>
                <label className="text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Assessment type<select aria-label="Assessment type" value={meta.assessment_type ?? 'other'} onChange={(e) => setMeta({ ...meta, assessment_type: e.target.value })} className={`${field} mt-1 font-normal normal-case tracking-normal`}>{ASSESSMENT_TYPES.map((t) => <option key={t} value={t}>{humanize(t)}</option>)}</select></label>
                <label className="text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Duration (minutes)<input aria-label="Duration minutes" type="number" min={1} value={meta.duration_minutes ?? ''} onChange={(e) => setMeta({ ...meta, duration_minutes: e.target.value ? Number(e.target.value) : null })} className={`${field} mt-1 font-normal normal-case tracking-normal`} /></label>
                <label className="text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Change summary<input aria-label="Change summary" value={meta.change_summary ?? ''} onChange={(e) => setMeta({ ...meta, change_summary: e.target.value })} className={`${field} mt-1 font-normal normal-case tracking-normal`} /></label>
                <label className="text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4] md:col-span-2">Instructions<textarea aria-label="Instructions" rows={2} value={meta.instructions ?? ''} onChange={(e) => setMeta({ ...meta, instructions: e.target.value || null })} className={`${field} mt-1 font-normal normal-case tracking-normal`} /></label>
                <p className="text-[11px] text-[#737373] md:col-span-2">Total marks are recomputed from the question marks on save. Saving a draft resets its validation and review state.</p>
              </Card>
            ...
            <>
              {v.instructions && <Card className="p-4"><h3 className="text-sm font-semibold text-[#111111] dark:text-white mb-1">Instructions</h3><p className="text-sm whitespace-pre-wrap text-[#262626] dark:text-[#D4D4D4]">{v.instructions}</p></Card>}
