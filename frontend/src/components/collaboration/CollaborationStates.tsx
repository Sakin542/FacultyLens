...
export const PermissionBadge: React.FC<{ permissions: Permissions | undefined; permission: PermissionKey; label: string }> = ({ permissions, permission, label }) => (
  <span data-testid={`permission-${permission}`} className={cn('inline-flex items-center gap-1 rounded-md border px-2 py-0.5 text-[11px]', permissions?.[permission] ? 'border-emerald-200 text-emerald-700 bg-emerald-50 dark:bg-emerald-950/30 dark:text-emerald-300' : 'border-sage-200 dark:border-[#2A2A2A] text-sage-400 line-through')}>
    {label}
  </span>
);

export const CollaborationLoading: React.FC<{ label?: string }> = ({ label = 'Loading collaboration…' }) => (
  <div data-testid="collaboration-loading" role="status" className="flex items-center gap-2 text-sm text-sage-500 px-4 py-6">
...
export const CollaborationEmptyState: React.FC<{ title: string; description: string; icon?: React.ReactNode; action?: React.ReactNode; className?: string }> = ({ title, description, icon, action, className }) => (
  <div data-testid="collaboration-empty-state" className={cn('flex flex-col items-center justify-center text-center py-10 px-6 rounded-xl border border-dashed border-sage-200 dark:border-[#2A2A2A]', className)}>
    <div className="w-12 h-12 rounded-full bg-sage-100 dark:bg-[#1F1F1F] border border-sage-200 dark:border-[#2A2A2A] flex items-center justify-center text-sage-500 mb-3">{icon ?? <Users className="w-6 h-6" />}</div>
    <h4 className="text-base font-semibold text-sage-800 dark:text-white mb-1">{title}</h4>
    <p className="text-sm text-sage-500 max-w-md mb-4">{description}</p>
    {action}
  </div>
);
...
export const CollaborationHeader: React.FC<{ courseCode?: string | null; courseName?: string | null; ownerName?: string | null; role: CollaborationRole | null; memberCount: number; permissions?: Permissions }> = ({ courseCode, courseName, ownerName, role, memberCount, permissions }) => (
  <header data-testid="collaboration-header" className="rounded-xl border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616] p-5 space-y-3">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div>
        <h1 className="text-xl font-bold text-sage-800 dark:text-white flex items-center gap-2"><Users className="w-5 h-5" /> Course Collaboration</h1>
        <p className="text-sm text-sage-500">{[courseCode, courseName].filter(Boolean).join(' — ')}{ownerName ? ` · Owner: ${ownerName}` : ''} · {memberCount} collaborator{memberCount === 1 ? '' : 's'}</p>
      </div>
      <div className="flex items-center gap-2 text-xs text-sage-500">Your role: <CollaboratorRoleBadge role={role} /></div>
    </div>
    ...
    <p className="text-[11px] text-sage-400">Collaboration provides controlled faculty review and discussion. AI findings remain assistive and never become decisions automatically.</p>
  </header>
);
