...
const Modal: React.FC<{ title: string; onClose: () => void; children: React.ReactNode; testId: string }> = ({ title, onClose, children, testId }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={title} data-testid={testId}>
    <div className="w-full max-w-md rounded-xl bg-white dark:bg-[#161616] border border-sage-200 dark:border-[#2A2A2A] p-5 space-y-4">
...
export const RoleSelect: React.FC<{ value: AssignableRole; onChange: (r: AssignableRole) => void; label?: string }> = ({ value, onChange, label = 'Role' }) => (
  <label className="block text-xs font-medium text-sage-600 dark:text-sage-400">
    ...
    <span className="mt-1 block text-[11px] text-sage-400">{ROLE_DESCRIPTIONS[value]}</span>
  </label>
);
...
export const CollaboratorList: React.FC<{ owner: { id: number; name: string; email?: string | null } | null; collaborators: Collaborator[]; permissions: Permissions; onInvite: () => void; onChangeRole: (c: Collaborator) => void; onRemove: (c: Collaborator) => void }> = ({ owner, collaborators, permissions, onInvite, onChangeRole, onRemove }) => {
  ...
  return (
    <section data-testid="collaborator-list" className="rounded-xl border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616]">
      <div className="flex items-center justify-between px-4 py-3 border-b border-sage-200 dark:border-[#2A2A2A]">
        <h2 className="text-sm font-semibold text-sage-800 dark:text-white">Collaborators</h2>
...
export const InvitationCard: React.FC<{ invitation: Invitation; onAccept?: () => void; onDecline?: () => void; onRevoke?: () => void; busy?: boolean }> = ({ invitation: i, onAccept, onDecline, onRevoke, busy }) => (
  <div data-testid={`invitation-${i.id}`} className="rounded-lg border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616] p-4 flex flex-wrap items-center justify-between gap-3">
    <div className="min-w-0">
      <p className="text-sm font-medium text-sage-800 dark:text-white flex items-center gap-2"><Mail className="w-4 h-4 text-sage-500" /> {[i.course?.course_code, i.course?.course_name].filter(Boolean).join(' — ') || 'Course'}</p>
      <p className="text-xs text-sage-500">{i.invited_email ? `To ${i.invited_email} · ` : ''}Invited by {i.invited_by?.name ?? 'a faculty member'} · Role: {ROLE_LABELS[i.role]}{i.expires_at ? ` · Expires ${new Date(i.expires_at).toLocaleDateString()}` : ''}</p>
      {i.message && <p className="mt-1 text-xs italic text-sage-600 dark:text-sage-400">“{i.message}”</p>}
    </div>
    <div className="flex items-center gap-2">
  ...
  return (
    <Modal title="Invite faculty" onClose={onClose} testId="invite-modal">
      <form noValidate className="space-y-3" onSubmit={(e) => { e.preventDefault(); if (valid) void onSubmit({ email: email.trim(), role, message: message.trim() || undefined }); }}>
        <label className="block text-xs font-medium text-sage-600 dark:text-sage-400">Faculty email<Input aria-label="Faculty email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="colleague@university.edu" className="mt-1" /></label>
        <RoleSelect value={role} onChange={setRole} />
        <label className="block text-xs font-medium text-sage-600 dark:text-sage-400">Message (optional)
