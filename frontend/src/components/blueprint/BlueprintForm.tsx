...
const selectCls = 'w-full rounded-md border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-1.5 text-sm';
const Label: React.FC<{ htmlFor: string; children: React.ReactNode }> = ({ htmlFor, children }) => <label htmlFor={htmlFor} className="block text-xs font-medium uppercase tracking-wider text-sage-600 dark:text-sage-400 mb-1">{children}</label>;
const Hint: React.FC<{ ok: boolean; children: React.ReactNode }> = ({ ok, children }) => <p role="status" className={`text-xs ${ok ? 'text-sage-500' : 'text-amber-800 dark:text-amber-300'}`}>{children}</p>;
...
export const BlueprintBasicSettings: React.FC<{ value: BlueprintInput; onChange: (v: BlueprintInput) => void; assessment: { title: string; type: string } | null }> = ({ value, onChange, assessment }) => (
  <Card data-testid="blueprint-basic-settings" className="p-4 space-y-3">
    <h3 className="text-sm font-semibold text-sage-800 dark:text-white">Basic settings</h3>
    {assessment && <p className="text-xs text-sage-500">Linked assessment: <strong>{assessment.title}</strong> ({humanize(assessment.type)}) — no duplicate assessment is created.</p>}
...
export const BlueprintSectionEditor: React.FC<{ value: BlueprintInput; onChange: (v: BlueprintInput) => void }> = ({ value, onChange }) => {
  ...
  return (
    <Card data-testid="blueprint-section-editor" className="p-4 space-y-3">
      ...
      {sections.map((s, i) => (
        <fieldset key={i} className="grid grid-cols-2 md:grid-cols-6 gap-2 items-end border-t border-sage-100 dark:border-[#2A2A2A] pt-2" data-testid={`section-row-${i}`}>
          ...
          <div className="text-sm tabular-nums"><span className="text-xs text-sage-500 block">Total</span>{s.question_count} × {fmtMarks(s.marks_per_question)} = <strong>{fmtMarks(s.question_count * s.marks_per_question)}</strong></div>
    ...
    <Card data-testid="blueprint-question-structure" className="p-4 space-y-2">
      ...
      {rows.map((r, i) => (
        <div key={i} className="grid grid-cols-2 md:grid-cols-5 gap-2 items-end">
          ...
          <div className="text-sm tabular-nums"><span className="text-xs text-sage-500 block">Total</span>{r.target_count} × {fmtMarks(r.marks_each)} = <strong>{fmtMarks(r.target_count * (r.marks_each ?? 0))}</strong></div>
    ...
    <Card data-testid={testId} className="p-4 space-y-2">
      ...
      <table className="w-full text-sm"><caption className="sr-only">{title} targets</caption>
        ...
        <tbody>{keys.map((k) => {
          ...
          return (
            <tr key={k} className="border-t border-sage-100 dark:border-[#2A2A2A]">
              ...
              {targets && <td className="py-1 text-xs text-sage-500">{targets[k] !== undefined ? `${targets[k]}%` : '—'}</td>}
              <td className="py-1 text-xs text-sage-500 tabular-nums">{mode === 'percentage' ? (derivedCount !== null ? `≈ ${derivedCount} of ${total} questions` : '') : (derivedPct !== null ? `${derivedPct}%` : '')}</td>
            </tr>
          );
        })}</tbody>
      </table>
...
export const BlueprintOutcomeDistribution: React.FC<{ value: BlueprintInput; onChange: (v: BlueprintInput) => void; outcomes: OutcomeOption[]; programOutcomes: ProgramOutcomeOption[] | null }> = ({ value, onChange, outcomes, programOutcomes }) => {
  ...
  return (
    <Card data-testid="blueprint-outcomes" className="p-4 space-y-3">
      ...
      {outcomes.length === 0 ? <p className="text-xs text-sage-500">This course has no learning outcomes yet.</p> : (
        <table className="w-full text-sm"><caption className="sr-only">Course outcome targets</caption>
          ...
          <tbody>{outcomes.map((o) => {
            ...
            return (
              <tr key={o.id} className="border-t border-sage-100 dark:border-[#2A2A2A]">
                <td className="py-1"><span className="font-medium">{o.code}</span><p className="text-xs text-sage-500 truncate max-w-xs" title={o.description}>{o.description}</p></td>
                ...
                <td className="py-1 text-xs text-sage-500 tabular-nums">{derived !== null ? `≈ ${fmtMarks(derived)} marks` : ''}</td>
      ...
      {!programOutcomes ? <p className="text-xs text-sage-500" data-testid="po-not-configured">PO blueprint is not configured for this course.</p> : programOutcomes.length === 0 ? <p className="text-xs text-sage-500">The linked program has no active outcomes.</p> : (
        <>
