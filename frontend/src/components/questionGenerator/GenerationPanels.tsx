...
export const GenerationRequestSummary: React.FC<{ request: GenerationRequest }> = ({ request: r }) => {
  ...
  return (
    <section data-testid="generation-request-summary" className="rounded-xl border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616] p-4 text-sm space-y-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="font-semibold text-sage-800 dark:text-white">Request #{r.id} <span className="text-sage-500 font-normal">· {[r.course?.course_code, r.course?.course_name].filter(Boolean).join(' — ')}{r.assessment ? ` · ${r.assessment.title}` : ''}</span></h3>
        <Badge variant={statusVariant} data-testid="generation-status">{fmt(r.generation_status)}</Badge>
      </div>
      <dl className="grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1 text-xs text-sage-600 dark:text-sage-400">
        <div><dt className="text-sage-400">Topic</dt><dd>{r.topic || 'Any'}</dd></div>
        <div><dt className="text-sage-400">CO / PO</dt><dd>{r.learning_outcome?.code || '—'}{r.program_outcome ? ` / ${r.program_outcome.code}` : ''}</dd></div>
        <div><dt className="text-sage-400">Constraints</dt><dd>{QUESTION_TYPE_LABELS[r.question_type] ?? r.question_type} · {fmt(r.difficulty_level) || 'Any'} · {r.cognitive_level || 'Any'} · {r.marks} marks × {r.number_of_questions}</dd></div>
        <div><dt className="text-sage-400">Models</dt><dd className="font-mono break-all">{r.models.generation || '—'}{r.models.prompt_version ? ` · prompt v${r.models.prompt_version}` : ''}</dd></div>
        <div><dt className="text-sage-400">Grounding</dt><dd>{r.retrieved_chunks} document passage{r.retrieved_chunks === 1 ? '' : 's'} · {r.existing_questions_count} existing question{r.existing_questions_count === 1 ? '' : 's'} checked</dd></div>
        <div><dt className="text-sage-400">Regenerations</dt><dd>{r.regeneration_count} / {r.max_regenerations}</dd></div>
        {r.assessment?.remaining_marks != null && <div><dt className="text-sage-400">Assessment marks remaining</dt><dd>{r.assessment.remaining_marks} of {r.assessment.total_marks}</dd></div>}
      </dl>
      {r.warnings.length > 0 && (
      ...
      {s && s.total > 0 && (
        <div className="rounded-lg bg-sage-100 dark:bg-[#1F1F1F] p-3 text-xs space-y-1" data-testid="set-summary">
          <p className="font-semibold text-sage-800 dark:text-white">Generated set overview <span className="font-normal text-sage-500">(review aid, not a quality verdict)</span></p>
...
export const QuestionGenerationHistory: React.FC<{ requests: GenerationRequest[]; activeId: number | null; onSelect: (r: GenerationRequest) => void }> = ({ requests, activeId, onSelect }) => (
  <aside data-testid="generation-history" className="rounded-xl border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616]">
    <div className="px-3 py-2 border-b border-sage-200 dark:border-[#2A2A2A] text-sm font-semibold text-sage-800 dark:text-white">Generation history</div>
    {requests.length === 0 ? <p className="px-3 py-4 text-xs text-sage-500">No previous generation requests.</p> : (
      <ul className="max-h-[420px] overflow-y-auto">
        {requests.map((r) => (
          <li key={r.id}>
            <button type="button" onClick={() => onSelect(r)} data-testid={`history-item-${r.id}`}
              className={cn('w-full text-left px-3 py-2 border-b border-sage-100 dark:border-[#1F1F1F] text-xs', activeId === r.id ? 'bg-sage-100 dark:bg-[#1F1F1F]' : 'hover:bg-sage-50 dark:hover:bg-[#161616]')}>
              <span className="block font-medium text-sage-800 dark:text-white truncate">{r.topic || 'Any topic'} · {QUESTION_TYPE_LABELS[r.question_type] ?? r.question_type} × {r.number_of_questions}</span>
              <span className="block text-sage-500 truncate">{r.course?.course_code}{r.assessment ? ` · ${r.assessment.title}` : ''}{r.learning_outcome ? ` · ${r.learning_outcome.code}` : ''}</span>
              <span className="block text-sage-400">{fmt(r.generation_status)} · {r.approved_count ?? 0}/{r.generated_questions_count ?? 0} approved</span>
            </button>
          </li>
        ))}
      </ul>
...
const Modal: React.FC<{ title: string; onClose: () => void; children: React.ReactNode; testId: string }> = ({ title, onClose, children, testId }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-label={title} data-testid={testId}>
    <div className="w-full max-w-lg rounded-xl bg-white dark:bg-[#161616] border border-sage-200 dark:border-[#2A2A2A] p-5 space-y-4">
      <div className="flex items-center justify-between"><h3 className="text-base font-semibold text-sage-800 dark:text-white">{title}</h3><button type="button" aria-label="Close" onClick={onClose} className="text-sage-500">✕</button></div>
      {children}
    </div>
  </div>
);
...
export const AddToAssessmentModal: React.FC<{
  ...
  return (
    <Modal title="Add to assessment" onClose={onClose} testId="add-to-assessment-modal">
      <p className="text-sm text-sage-600 dark:text-sage-400">This action will create an <strong>official assessment question</strong>. The draft itself stays in the generator history.</p>
      <div className="rounded-lg bg-sage-100 dark:bg-[#1F1F1F] p-3 text-sm space-y-1">
        <p className="text-sage-800 dark:text-white whitespace-pre-wrap">{question.question_text}</p>
        <p className="text-xs text-sage-500">Marks: {question.marks} · CO: {outcomeCode || '—'} · Difficulty: {fmt(question.difficulty_level)} · Bloom: {question.cognitive_level || '—'}</p>
      </div>
      <label className="block text-xs font-medium text-sage-600 dark:text-sage-400">
        Assessment
        <select aria-label="Target assessment" className="mt-1 w-full rounded-lg border border-sage-200 dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-3 py-2 text-sm" value={assessmentId} onChange={(e) => setAssessmentId(e.target.value)}>
...
export const RegenerateQuestionModal: React.FC<{
  ...
  return (
    <Modal title={title} onClose={onClose} testId="regenerate-modal">
      <p className="text-sm text-sage-600 dark:text-sage-400">Tell the generator what to change. The current draft is kept in history as rejected. {regenerationsLeft} regeneration{regenerationsLeft === 1 ? '' : 's'} left for this request.</p>
