    ...
    <div className="space-y-4" data-testid="rubric-preview">
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="space-y-1">
          ...
          <h3 className="text-sm font-bold text-[#111111] dark:text-white">{rubric.title}</h3>
          {rubric.status === 'DRAFT' && (
        ...
        <div className="text-right">
          <p className="text-[10px] uppercase tracking-wider text-[#737373]">Total Marks</p>
          <p
            className={`text-sm font-mono font-bold ${isConsistent ? 'text-[#111111] dark:text-white' : 'text-red-600'}`}
      ...
      {questionText && (
        <div className="p-3 bg-[#F7F7F5] dark:bg-[#2C2C2E] rounded-xl border border-[#E5E5E5] dark:border-[#3A3A3C]">
          <p className="text-[10px] uppercase tracking-wider text-[#737373] mb-1">Question</p>
          <p className="text-xs text-[#262626] dark:text-[#E5E5E5] leading-relaxed">{questionText}</p>
        </div>
      )}
      ...
      <ol className="space-y-2">
        {rubric.criteria.map((c, idx) => (
          <li
            key={c.id ?? idx}
            className="p-3 rounded-xl border border-[#E5E5E5] dark:border-[#3A3A3C] bg-white dark:bg-[#1C1C1E] space-y-1.5"
            data-testid="rubric-criterion"
          >
            <div className="flex items-start justify-between gap-3">
              <div className="space-y-0.5">
                <p className="text-[10px] uppercase tracking-wider text-[#737373]">Suggested criterion {c.sort_order}</p>
                <p className="text-xs font-bold text-[#111111] dark:text-white">{c.criterion}</p>
              </div>
              <span className="text-xs font-mono font-bold text-[#111111] dark:text-white whitespace-nowrap">
                {formatMarks(c.max_marks)} {Number(c.max_marks) === 1 ? 'mark' : 'marks'}
              </span>
            </div>
            <p className="text-xs text-[#262626] dark:text-[#E5E5E5] leading-relaxed">{c.description}</p>
            {c.scoring_guidance && (
              <p className="text-[11px] text-[#737373] leading-relaxed">
                <span className="font-semibold text-[#111111] dark:text-white">Scoring guidance: </span>
                {c.scoring_guidance}
              </p>
            )}
            {c.expected_indicators && c.expected_indicators.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
                <span className="text-[10px] font-semibold text-[#111111] dark:text-white">Expected indicators:</span>
                {c.expected_indicators.map((ind, i) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 rounded bg-[#F7F7F5] dark:bg-[#2C2C2E] border border-[#E5E5E5] dark:border-[#3A3A3C] text-[10px] text-[#262626] dark:text-[#E5E5E5]"
                  >
                    {ind}
                  </span>
                ))}
      ...
      {rubric.general_guidance && (
        <div className="p-3 bg-[#F7F7F5] dark:bg-[#2C2C2E] rounded-xl border border-[#E5E5E5] dark:border-[#3A3A3C]">
          <p className="text-[10px] uppercase tracking-wider text-[#737373] mb-1">General guidance</p>
          <p className="text-xs text-[#262626] dark:text-[#E5E5E5] leading-relaxed">{rubric.general_guidance}</p>
        </div>
      )}
      ...
      {/* Actions */}
      {!isReadOnly && (
        <div className="flex flex-wrap items-center justify-end gap-2 pt-2 border-t border-[#E5E5E5] dark:border-[#2C2C2E]">
          {onDelete && (
