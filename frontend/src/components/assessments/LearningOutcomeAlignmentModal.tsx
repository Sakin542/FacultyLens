          ...
          <div className="bg-sage-100 rounded-xl p-4 border border-sage-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-4 text-xs text-sage-600">
              <div className="flex items-center gap-1.5 font-medium">
                <BookOpen className="w-4 h-4 text-sage-800" />
          ...
          {showThresholdSettings && (
            <div className="p-4 bg-white border border-sage-200 rounded-xl space-y-3 shadow-subtle animate-in slide-in-from-top-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-sage-800 uppercase tracking-wider">Semantic Match Thresholds</span>
                <span className="text-[11px] text-sage-500">Cosine Similarity Range: 0.0 - 1.0</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-xs text-sage-500 block mb-1">
                    Strong Alignment Threshold: <strong className="text-sage-800 font-mono">{strongThreshold}</strong>
                  </label>
                  <input
                    type="range"
                    min="0.50"
                  ...
                  />
                  <span className="text-[10px] text-sage-400">High semantic certainty (recommended 0.70)</span>
                </div>
                <div>
                  <label className="text-xs text-sage-500 block mb-1">
                    Weak Alignment Threshold: <strong className="text-sage-800 font-mono">{weakThreshold}</strong>
                  </label>
                  <input
                    type="range"
                    min="0.30"
                    ...
                    className="w-full accent-sage-700"
                  />
                  <span className="text-[10px] text-sage-400">Partial / secondary relevance (recommended 0.50)</span>
                </div>
              </div>
            </div>
          )}
          ...
          {alignmentResult ? (
            <div className="space-y-6 animate-in fade-in duration-300">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 bg-white border border-sage-200 rounded-xl flex flex-col justify-between shadow-subtle">
                  <span className="text-[11px] uppercase font-bold text-sage-500 tracking-wider">Overall Alignment</span>
                ...
                <div className="p-4 bg-white border border-sage-200 rounded-xl flex flex-col justify-between shadow-subtle">
                  <span className="text-[11px] uppercase font-bold text-sage-500 tracking-wider">LO Coverage</span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-3xl font-black font-mono text-sage-800">
                      {alignmentResult.covered_learning_outcomes_count} / {alignmentResult.total_learning_outcomes}
                    </span>
                    <span className="text-xs font-medium text-sage-500">Outcomes Met</span>
                  </div>
                  <div className="mt-3">
                    <ProgressBar
                ...
                <div className="p-4 bg-white border border-sage-200 rounded-xl flex flex-col justify-between shadow-subtle">
                  <span className="text-[11px] uppercase font-bold text-sage-500 tracking-wider">Embedding Engine</span>
                  <div className="mt-1">
                    <div className="text-xs font-mono font-semibold text-sage-800 truncate">all-MiniLM-L6-v2</div>
                    <div className="text-[11px] text-sage-500 mt-0.5">384-dim Dense Vectors</div>
                  </div>
                  <div className="mt-2 text-[10px] text-sage-400 flex items-center gap-1">
                  ...
                  <ul className="space-y-1.5 text-xs text-[#404040]">
                    {alignmentResult.findings.map((f, idx) => (
                      <li key={idx} className="flex items-start gap-2">
                        <span className="text-sage-800 font-bold font-mono shrink-0">•</span>
                        <span>{f}</span>
                      </li>
                    ))}
                  </ul>
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-sage-800 uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="text-sage-500 font-normal lowercase">{alignmentResult.learning_outcome_coverage.length} learning outcomes</span>
                </h3>
                      ...
                      >
                        <div className="flex-1 space-y-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-sage-800">{lo.description}</span>
                          </div>
                          {lo.matching_questions_count > 0 && (
                            <div className="text-[11px] text-sage-500">
                              Assessed in Questions: <strong className="text-sage-800">{lo.matching_question_numbers.map(n => `Q${n}`).join(', ')}</strong> (Max sim: {(lo.max_similarity * 100).toFixed(1)}%)
                            </div>
                          )}
                        </div>
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-sage-800 uppercase tracking-wider flex items-center justify-between">
                  ...
                  <span className="text-sage-500 font-normal lowercase">{alignmentResult.question_alignment.length} questions mapped</span>
                </h3>
                ...
                <div className="space-y-3">
                  {alignmentResult.question_alignment.map((qa, idx) => {
                    ...
                    return (
                      <div
                        key={qKey}
                        className="p-4 bg-white border border-sage-200 rounded-xl space-y-2.5 shadow-2xs"
                      >
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
              ...
              <div className="max-w-md">
                <h3 className="text-sm font-bold text-sage-800">Ready for Alignment Analysis</h3>
