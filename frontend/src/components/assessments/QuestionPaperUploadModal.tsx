...
export const QuestionPaperUploadModal: React.FC<QuestionPaperUploadModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!file) {
      setError('Please select a question paper file to upload.');
      return;
    }
      ...
      <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl border border-sage-200 dark:border-[#2C2C2E] shadow-xl max-w-lg w-full p-6 space-y-5 animate-in fade-in zoom-in duration-150">
        <div className="flex items-center justify-between pb-3 border-b border-sage-200 dark:border-[#2C2C2E]">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-sage-800 dark:text-white">
              {currentFileName ? 'Replace Question Paper' : 'Upload Question Paper'}
            </h3>
          </div>
          <button
            type="button"
        ...
        {currentFileName && (
          <div className="p-3 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl text-xs text-amber-800 dark:text-amber-300">
            <p className="font-semibold">Current Question Paper:</p>
            <p className="font-mono mt-0.5 truncate">{currentFileName}</p>
            <p className="text-[11px] mt-1 text-amber-700 dark:text-amber-400">
              Uploading a new file will safely replace the existing question paper.
            </p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {file ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs font-semibold text-sage-800 dark:text-white">
                  Click to choose question paper or drag & drop here
                </p>
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-sage-200 dark:border-[#2C2C2E]">
            ...
            >
              {isSubmitting ? 'Uploading...' : currentFileName ? 'Replace Paper' : 'Upload Paper'}
            </Button>
          </div>
        </form>
      </div>
