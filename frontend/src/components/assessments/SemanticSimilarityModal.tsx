...
export const SemanticSimilarityModal: React.FC<SemanticSimilarityModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div
        className="bg-white rounded-2xl max-w-4xl w-full max-h-[92vh] flex flex-col shadow-2xl border border-sage-200 animate-in fade-in zoom-in-95 duration-200"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-sage-200">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-sage-700 text-white flex items-center justify-center shadow-subtle">
              <CopyCheck className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base font-bold text-sage-800 tracking-tight">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-sm">
          ...
          <div className="bg-sage-100 rounded-xl p-4 border border-sage-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-4 text-xs text-sage-600">
              <div className="flex items-center gap-1.5 font-medium">
                <Archive className="w-4 h-4 text-sage-800" />
                <span><strong>{bankQuestionsCount}</strong> Historical Questions in Bank</span>
              </div>
              <div className="flex items-center gap-1.5 font-medium">
                <HelpCircle className="w-4 h-4 text-sage-800" />
          ...
          {showSettings && (
            <div className="p-4 bg-white border border-sage-200 rounded-xl space-y-3 shadow-subtle animate-in slide-in-from-top-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-sage-800 uppercase tracking-wider">Configurable Similarity Cutoffs</span>
                <span className="text-[11px] text-sage-500">Cosine Distance Range: 0.0 - 1.0</span>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div>
                  <label className="text-xs text-sage-500 block mb-1">
                    Potential Duplicate: <strong className="text-sage-800 font-mono">{duplicateThreshold}</strong>
                  </label>
                  <input
                    type="range"
                    min="0.75"
                  ...
                  />
                  <span className="text-[10px] text-sage-400">Threshold for repetition alert</span>
                </div>
                <div>
                  <label className="text-xs text-sage-500 block mb-1">
                    Highly Similar: <strong className="text-sage-800 font-mono">{highThreshold}</strong>
                  </label>
                  <input
                    type="range"
                    min="0.60"
                    ...
                    className="w-full accent-sage-700"
                  />
                  <span className="text-[10px] text-sage-400">High concept & phrasing overlap</span>
                </div>
                <div>
                  <label className="text-xs text-sage-500 block mb-1">
                    Moderate Similar: <strong className="text-sage-800 font-mono">{moderateThreshold}</strong>
                  </label>
                  <input
                    type="range"
                    min="0.40"
                    ...
                    className="w-full accent-sage-700"
                  />
                  <span className="text-[10px] text-sage-400">Moderate topical overlap</span>
                </div>
                <div>
                  <label className="text-xs text-sage-500 block mb-1">
                    Top-K Matches: <strong className="text-sage-800 font-mono">{topK}</strong>
                  </label>
                  <input
                    type="number"
                    min="1"
                    ...
                    className="w-full px-2.5 py-1 text-xs border border-sage-200 rounded-lg"
                  />
                  <span className="text-[10px] text-sage-400">Ranked matches to return</span>
                </div>
              </div>
            </div>
          )}
          ...
          {similarityResult ? (
            <div className="space-y-6 animate-in fade-in duration-300">
              {/* Summary Cards */}
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div className="p-4 bg-white border border-sage-200 rounded-xl flex flex-col justify-between shadow-subtle">
                  <span className="text-[11px] uppercase font-bold text-sage-500 tracking-wider">Compared</span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-3xl font-black font-mono text-sage-800">
                      {similarityResult.total_current_questions}
                    </span>
                    <span className="text-xs text-sage-500">Questions</span>
                  </div>
                  <span className="text-[11px] text-sage-500 mt-2">vs {similarityResult.total_previous_questions} in bank</span>
                </div>

                <div className={`p-4 rounded-xl flex flex-col justify-between shadow-subtle border ${similarityResult.potential_duplicates_count > 0 ? 'bg-[#FEF2F2] border-[#FECACA]' : 'bg-white border-sage-200'}`}>
                  <span className={`text-[11px] uppercase font-bold tracking-wider ${similarityResult.potential_duplicates_count > 0 ? 'text-[#991B1B]' : 'text-sage-500'}`}>
                    Potential Duplicates
                  </span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className={`text-3xl font-black font-mono ${similarityResult.potential_duplicates_count > 0 ? 'text-[#DC2626]' : 'text-sage-800'}`}>
                      {similarityResult.potential_duplicates_count}
                    </span>
                    <span className="text-xs text-sage-500">(&gt;= {Math.round(duplicateThreshold * 100)}%)</span>
                  </div>
                  <span className="text-[11px] text-sage-500 mt-2">Requires faculty review</span>
                </div>

                <div className="p-4 bg-white border border-sage-200 rounded-xl flex flex-col justify-between shadow-subtle">
                  <span className="text-[11px] uppercase font-bold text-sage-500 tracking-wider">Highly Similar</span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-3xl font-black font-mono text-sage-800">
                      {similarityResult.highly_similar_count}
                    </span>
                    <span className="text-xs text-sage-500">Items</span>
                  </div>
                  <span className="text-[11px] text-sage-500 mt-2">{Math.round(highThreshold * 100)}% - {Math.round(duplicateThreshold * 100)}% overlap</span>
                </div>

                <div className="p-4 bg-white border border-sage-200 rounded-xl flex flex-col justify-between shadow-subtle">
                  <span className="text-[11px] uppercase font-bold text-sage-500 tracking-wider">Avg Similarity</span>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-3xl font-black font-mono text-sage-800">
              ...
              {similarityResult.findings && similarityResult.findings.length > 0 && (
                <div className="p-4 bg-sage-100 border border-sage-200 rounded-xl space-y-2">
              ...
              {similarityResult.results.some(r => r.max_similarity_status === 'POTENTIAL_DUPLICATE' || r.max_similarity_status === 'HIGHLY_SIMILAR') && (
                <div className="space-y-3">
                  <h3 className="text-xs font-bold text-sage-800 uppercase tracking-wider flex items-center gap-2">
                    ...
                    {similarityResult.results
                      ...
                      .map((item, idx) => {
                        ...
                        return (
                          <div
                            key={qId}
                            className={`p-4 rounded-xl border transition-colors space-y-3 ${decision === 'accepted' ? 'bg-[#F0FDF4] border-[#86EFAC]' : item.max_similarity_status === 'POTENTIAL_DUPLICATE' ? 'bg-[#FEF2F2]/50 border-[#FECACA]' : 'bg-white border-sage-200'}`}
                          >
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-sage-800 uppercase tracking-wider flex items-center justify-between">
                  <span>Question-by-Question Similarity Overview</span>
                  <span className="text-sage-500 font-normal lowercase">{similarityResult.results.length} questions compared</span>
                </h3>
                ...
                <div className="space-y-2.5">
                  {similarityResult.results.map((item, idx) => {
                    ...
                    return (
                      <div
                        key={qKey}
                        className="p-3.5 bg-white border border-sage-200 rounded-xl space-y-2 shadow-2xs"
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-2.5 flex-1">
                            <span className="px-2 py-0.5 bg-sage-100 border border-sage-200 rounded text-xs font-mono font-bold text-sage-800 shrink-0">
                              Q{item.current_question_number}
                            </span>
                        ...
                        {bestMatch && bestMatch.similarity_score >= moderateThreshold && (
                          <div className="p-2.5 bg-sage-100 rounded-lg text-xs flex items-center justify-between gap-2 border border-sage-200">
                        ...
                        {item.matches && item.matches.length > 1 && (
                          <div className="pt-0.5">
                            ...
                            {isExpanded && (
                              <div className="mt-2 space-y-1.5 pl-3 border-l-2 border-sage-200">
            ...
            <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
              <div className="w-12 h-12 rounded-2xl bg-sage-100 border border-sage-200 flex items-center justify-center text-sage-500">
                <CopyCheck className="w-6 h-6" />
              </div>
              <div className="max-w-md">
                <h3 className="text-sm font-bold text-sage-800">Ready for Semantic Similarity Check</h3>
          ...
          <div className="pt-2 border-t border-sage-200 flex items-center gap-2 text-[11px] text-sage-400">
        ...
        {/* Footer */}
        <div className="px-6 py-3.5 border-t border-sage-200 bg-sage-50 rounded-b-2xl flex items-center justify-between">
