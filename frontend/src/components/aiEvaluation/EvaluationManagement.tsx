...
export const DatasetValidation: React.FC<{ report: DatasetValidationReport | null; datasetStatus: EvaluationDataset['status'] }> = ({ report, datasetStatus }) => {
  if (!report) return <p data-testid="dataset-validation" className="text-sm text-[#737373]">Not validated yet. Validate the dataset before running an evaluation.</p>;
  return (
    <div data-testid="dataset-validation" className="space-y-2">
      ...
      {Object.keys(report.label_distribution).length > 0 && (
        <p className="text-xs text-[#737373]">Label distribution: {Object.entries(report.label_distribution).map(([l, c]) => `${l} ${c}`).join(' · ')}</p>
      )}
  ...
  return (
    <Card data-testid="dataset-manager" className="p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-[#111111] dark:text-white">Evaluation datasets</h3>
        <div className="flex items-center gap-2">
          <label htmlFor="dataset-task-filter" className="sr-only">Filter datasets by task</label>
          <select id="dataset-task-filter" value={taskFilter} onChange={(e) => onTaskFilter(e.target.value as EvaluationTask | '')} className="rounded-md border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-1 text-sm">
      ...
      {open && (
        <form onSubmit={submit} noValidate data-testid="dataset-form" className="rounded-lg border border-[#E5E5E5] dark:border-[#2A2A2A] p-3 space-y-3 bg-[#FAFAF8] dark:bg-[#1A1A1A]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <label htmlFor="ds-task" className="block text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Task</label>
              <select id="ds-task" value={task} onChange={(e) => setTask(e.target.value as EvaluationTask)} className="w-full rounded-md border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-2 text-sm">
                {EVALUATION_TASKS.map((t) => <option key={t} value={t}>{TASK_LABELS[t]}</option>)}
              </select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label htmlFor="ds-source" className="block text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Ground-truth source</label>
                <select id="ds-source" value={source} onChange={(e) => setSource(e.target.value as DatasetSource)} className="w-full rounded-md border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-2 text-sm">
                  {SOURCES.map((s) => <option key={s} value={s}>{statusText(s)}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                <label htmlFor="ds-split" className="block text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Split</label>
                <select id="ds-split" value={split} onChange={(e) => setSplit(e.target.value as DatasetSplit)} className="w-full rounded-md border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-2 text-sm">
          ...
          <div className="space-y-1.5">
            <label htmlFor="ds-examples" className="block text-xs font-medium uppercase tracking-wider text-[#262626] dark:text-[#D4D4D4]">Examples (JSON array, faculty-validated ground truth)</label>
            <textarea id="ds-examples" value={examples} onChange={(e) => setExamples(e.target.value)} rows={5} placeholder={EXAMPLE_HINTS[task]} className="w-full rounded-md border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-2 text-xs font-mono" />
            <p className="text-xs text-[#737373]">Held-out TEST examples are recommended. Do not evaluate only on data used to tune prompts or rules.</p>
          </div>
      ...
      {datasets.length === 0 ? (
        <p className="text-sm text-[#737373]">No evaluation datasets yet.</p>
      ) : (
        <ul className="divide-y divide-[#F0F0F0] dark:divide-[#2A2A2A]" aria-label="Evaluation datasets">
          {datasets.map((d) => (
            <li key={d.id} className={cn('py-2 flex flex-wrap items-center gap-2', selected?.id === d.id && 'bg-[#FAFAF8] dark:bg-[#1A1A1A] -mx-2 px-2 rounded')}>
              <button type="button" onClick={() => onSelect(d)} className="flex-1 text-left min-w-0" aria-current={selected?.id === d.id ? 'true' : undefined}>
                <p className="text-sm font-medium text-[#111111] dark:text-white truncate">{d.name} <span className="text-xs text-[#737373]">{d.version}</span></p>
                <p className="text-xs text-[#737373]">{TASK_LABELS[d.task]} · {d.examples_count ?? 0} examples · {statusText(d.source)} · {d.split}</p>
              </button>
      ...
      {selected && <div className="pt-2 border-t border-[#F0F0F0] dark:border-[#2A2A2A]"><DatasetValidation report={selected.validation_report} datasetStatus={selected.status} /></div>}
    </Card>
  );
};
...
export const EvaluationRunHistory: React.FC<HistoryProps> = ({ runs, selectedId, onOpen, compareIds, onToggleCompare, taskFilter, onTaskFilter }) => (
  <Card data-testid="evaluation-run-history" className="p-4 space-y-3">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <h3 className="text-sm font-semibold text-[#111111] dark:text-white">Evaluation history</h3>
      <div className="flex items-center gap-2">
        <label htmlFor="run-task-filter" className="sr-only">Filter runs by task</label>
        <select id="run-task-filter" value={taskFilter} onChange={(e) => onTaskFilter(e.target.value as EvaluationTask | '')} className="rounded-md border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#161616] px-2 py-1 text-sm">
          <option value="">All tasks</option>{EVALUATION_TASKS.map((t) => <option key={t} value={t}>{TASK_LABELS[t]}</option>)}
        </select>
      </div>
    </div>
    {runs.length === 0 ? <p className="text-sm text-[#737373]">No evaluation runs yet.</p> : (
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {runs.map((r) => (
              <tr key={r.id} className={cn('border-t border-[#F0F0F0] dark:border-[#2A2A2A]', selectedId === r.id && 'bg-[#FAFAF8] dark:bg-[#1A1A1A]')}>
                ...
                <td className="py-1 pr-3 text-xs text-[#737373]">{r.created_at ? new Date(r.created_at).toLocaleDateString() : '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export const EvaluationRunComparison: React.FC<{ comparison: RunComparison | null; loading?: boolean; onClear: () => void }> = ({ comparison, loading, onClear }) => {
  if (loading) return <Card className="p-4 text-sm text-[#737373]" data-testid="evaluation-run-comparison">Comparing runs…</Card>;
  if (!comparison) return null;
  const { run_a: a, run_b: b } = comparison;
  return (
    <Card data-testid="evaluation-run-comparison" className="p-4 space-y-3">
      <div className="flex items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-[#111111] dark:text-white">Run comparison: #{a.id} vs #{b.id}</h3>
      ...
      <p className="text-xs text-[#737373]">A: {a.model?.name ?? 'no model'} ({a.prompt_version ? a.prompt_version.version : 'no prompt'}) on {a.dataset?.name ?? '—'} · B: {b.model?.name ?? 'no model'} ({b.prompt_version ? b.prompt_version.version : 'no prompt'}) on {b.dataset?.name ?? '—'}</p>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <caption className="sr-only">Metric comparison between two runs</caption>
          <thead><tr className="text-left text-xs uppercase text-[#737373]"><th className="py-1 pr-3">Metric</th><th className="py-1 pr-3">Run #{a.id}</th><th className="py-1 pr-3">Run #{b.id}</th><th className="py-1 pr-3">Δ</th><th className="py-1 pr-3">Direction</th></tr></thead>
          <tbody>
            {comparison.rows.map((r) => (
              <tr key={r.metric} className={cn('border-t border-[#F0F0F0] dark:border-[#2A2A2A]', r.metric === comparison.headline_metric && 'font-semibold')}>
      ...
      <p className="text-xs text-[#737373]">{comparison.note}</p>
    </Card>
  );
};
