...
export const VersionBlueprintSummary: React.FC<{ blueprint: AssessmentVersionBlueprint | null; profile: VersionProfile | null }> = ({ blueprint, profile }) => (
  <Card data-testid="version-blueprint-summary" className="p-4 space-y-3">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <h3 className="text-sm font-semibold text-sage-800 dark:text-white">Blueprint snapshot</h3>
      {blueprint ? <span className="text-xs text-sage-500">Blueprint v{blueprint.blueprint_version} · {humanize(blueprint.blueprint_status)} · {blueprint.question_count} questions / {fmtMarks(blueprint.total_marks)} marks</span> : <span className="text-xs text-sage-500">No blueprint snapshot attached to this version.</span>}
    </div>
    {(blueprint || profile) && (
        ...
        {DIMS.map(([pk, bk, label]) => {
          ...
          return (
            <table key={pk} className="w-full text-xs" data-testid={`blueprint-dim-${pk}`}>
              <caption className="text-left text-[11px] uppercase tracking-wide text-sage-500 mb-1">{label}</caption>
              <thead><tr className="text-sage-500"><th className="text-left font-medium py-0.5">Key</th><th className="text-right font-medium">Planned</th><th className="text-right font-medium">Actual</th></tr></thead>
              <tbody>
                {keys.map((k) => (
                  <tr key={k} className="border-t border-sage-100 dark:border-[#2A2A2A]">
                    <td className="py-0.5 text-sage-700 dark:text-sage-300">{planned[k]?.label ?? actual[k]?.label ?? k}</td>
    ...
    {blueprint && blueprint.sections.length > 0 && (
      <p className="text-xs text-sage-500">Sections: {blueprint.sections.map((s) => `${s.title} (${s.question_count} × ${fmtMarks(s.marks_per_question)})`).join(' · ')}</p>
    )}
  </Card>
);
