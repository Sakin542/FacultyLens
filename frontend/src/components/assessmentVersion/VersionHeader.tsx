...
export const VersionHeader: React.FC<{
  ...
  <header data-testid="version-header" className="space-y-1">
    <nav aria-label="Breadcrumb" className="text-xs text-sage-500 flex flex-wrap gap-1">
    ...
    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <h1 className="text-2xl font-bold text-sage-800 dark:text-white">{title}</h1>
        {version && <VersionStatusBadge status={version.status} size="md" label={`${humanize(version.status)} · ${version.version_label}`} />}
      </div>
      {actions}
    </div>
    {course && assessment && <p className="text-sm text-sage-500">{course.code} — {course.name} · {assessment.title}{assessment.type ? ` (${humanize(assessment.type)})` : ''}</p>}
    <p className="text-xs text-sage-500 max-w-3xl">{subtitle ?? 'Every version is a snapshot of the assessment at a point in time. Finalized versions are immutable; to change anything, create a new version.'}</p>
  </header>
);
