...
export const VersionCreateDialog: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="dialog" aria-modal="true" aria-labelledby="version-create-title" data-testid="version-create-dialog">
      <form noValidate onSubmit={submit} className="w-full max-w-lg rounded-xl bg-white dark:bg-[#161616] border border-sage-200 dark:border-[#2A2A2A] p-5 space-y-4 shadow-lg">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="version-create-title" className="text-lg font-semibold text-sage-800 dark:text-white">{first ? 'Create version 1.0' : 'Create new version'}</h2>
            <p className="text-xs text-sage-500 mt-0.5">{first ? 'Snapshots the current questions, blueprint and approved rubrics of this assessment.' : 'Clones the selected version into a new draft. The source version is preserved unchanged.'}</p>
          </div>
        ...
        {!first && (
          <label className="block text-xs font-medium uppercase tracking-wider text-sage-700 dark:text-sage-300">Based on
            <select value={basedOn} onChange={(e) => setBasedOn(e.target.value)} className="mt-1 w-full rounded-lg border border-sage-200 bg-white dark:bg-sage-700 px-3 py-2 text-sm font-normal normal-case tracking-normal">
        ...
        <fieldset className="space-y-1">
          <legend className="text-xs font-medium uppercase tracking-wider text-sage-700 dark:text-sage-300">Version type</legend>
          ...
          <p className="text-[11px] text-sage-500">Labels follow deterministic rules: major → next vN.0, minor → next vM.n within the source's major line.</p>
        </fieldset>
        <label className="block text-xs font-medium uppercase tracking-wider text-sage-700 dark:text-sage-300">Change summary{!first && <span className="text-[#DC2626] ml-1">*</span>}
          <textarea value={summary} onChange={(e) => setSummary(e.target.value)} onBlur={() => setTouched(true)} rows={3} placeholder={first ? 'Optional note for the initial snapshot' : 'e.g. Updated Q3 and Q6 to improve CO2 coverage. Adjusted hard-question distribution.'}
            className="mt-1 w-full rounded-lg border border-sage-200 bg-white dark:bg-sage-700 px-3 py-2 text-sm font-normal normal-case tracking-normal" aria-invalid={touched && summaryMissing} />
