  ...
  return (
    <Card data-testid="question-change-list" className="p-4 space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 className="text-sm font-semibold text-[#111111] dark:text-white">Question changes</h3>
        <div role="tablist" aria-label="Filter question changes" className="flex flex-wrap gap-1">
          {FILTERS.map((f) => {
            ...
            return <button key={f} role="tab" aria-selected={filter === f} type="button" onClick={() => setFilter(f)} className={`text-xs px-2 py-1 rounded-md border ${filter === f ? 'bg-[#111111] text-white border-[#111111]' : 'border-[#E5E5E5] text-[#525252] hover:bg-[#F7F7F5]'}`}>{f === 'ALL' ? 'All' : f.charAt(0) + f.slice(1).toLowerCase()} ({n})</button>;
          })}
        </div>
      </div>
      {visible.length === 0 ? <p className="text-sm text-[#737373]">No questions in this category.</p> : (
        <ul className="space-y-3">
          {visible.map((i, idx) => (
            <li key={`${i.status}-${i.question_number}-${idx}`} data-testid={`question-change-${i.question_number}`} className="rounded-lg border border-[#E5E5E5] dark:border-[#2A2A2A] p-3 space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm font-semibold text-[#111111] dark:text-white">Question {i.question_number}</span>
                <Badge variant={statusVariant(i.status)} dot>{i.status}</Badge>
                {i.replaced && <Badge variant="Attention">Replaced question</Badge>}
                {i.status === 'MODIFIED' && <span className="text-xs text-[#737373]">{i.changes.length} field{i.changes.length === 1 ? '' : 's'} changed</span>}
              </div>
              {i.status === 'UNCHANGED' ? <p className="text-sm text-[#525252] dark:text-[#A3A3A3] whitespace-pre-wrap">{i.to?.question_text}</p>
