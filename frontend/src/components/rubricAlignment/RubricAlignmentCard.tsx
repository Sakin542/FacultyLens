  ...
  >
    <header className="flex flex-wrap items-center justify-between gap-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {alignment.rubric_version && <span className="text-[10px] text-sage-500">Rubric v{alignment.rubric_version}</span>}
        {!readOnly && (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      <div>
        <span className="block text-[10px] uppercase tracking-wider text-sage-500 mb-1">Strengths</span>
        ...
        ) : <p className="text-xs text-sage-500 italic">No strongly aligned criteria were identified.</p>}
      </div>
      <MissingElements items={alignment.missing_elements} title="Missing across criteria" />
    </div>

    <p className="text-[11px] text-sage-500">{ALIGNMENT_NOT_CORRECTNESS}</p>

    <div className="pt-3 border-t border-sage-200 dark:border-[#2C2C2E] flex flex-wrap items-center justify-between gap-2">
