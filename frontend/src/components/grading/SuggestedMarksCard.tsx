...
export const SuggestedMarksCard: React.FC<SuggestedMarksCardProps> = ({ result, facultyMarks, alignment }) => {
  ...
  return (
    <div className={`grid grid-cols-1 ${showAlignment ? 'sm:grid-cols-3' : 'sm:grid-cols-2'} gap-3`} data-testid="suggested-marks-card">
      <div className="p-4 rounded-xl bg-sage-100 dark:bg-[#2C2C2E] border border-sage-200 dark:border-[#3A3A3C]">
      ...
      <div className="p-4 rounded-xl bg-white dark:bg-[#1C1C1E] border border-sage-200 dark:border-[#3A3A3C]">
        ...
        {diff !== null && (
          <p className="text-[10px] text-sage-500 mt-1" data-testid="marks-difference">
            Difference from AI suggestion: {diff > 0 ? '+' : ''}{formatMarks(diff)}
          </p>
        )}
        {!hasFaculty && <p className="text-[10px] text-sage-500 mt-1">Awaiting faculty review.</p>}
      </div>
      {showAlignment && (
        <div className="p-4 rounded-xl bg-white dark:bg-[#1C1C1E] border border-sage-200 dark:border-[#3A3A3C]" data-testid="alignment-signal">
          <span className="text-[10px] uppercase tracking-wider font-semibold text-sage-500 flex items-center gap-1.5">
            <Target className="w-3 h-3" /> Answer ↔ Rubric Alignment
          </span>
          <p className="mt-2 text-2xl font-bold font-mono text-sage-800 dark:text-white">
          ...
          <p className="text-[10px] text-sage-500 mt-1">Coverage signal, not a grade.</p>
        </div>
      )}
    </div>
  );
