...
export const CourseOutcomeList: React.FC<{ outcomes: CourseOutcome[] }> = ({ outcomes }) => (
  <div className="space-y-1.5" data-testid="course-outcome-list">
    <h4 className="text-[10px] uppercase tracking-wider font-semibold text-sage-500">Course Outcomes ({outcomes.length})</h4>
    {outcomes.length === 0 ? (
      <p className="text-xs text-sage-500 italic">No learning outcomes yet. Add them in Course Details.</p>
    ) : (
      <ul className="divide-y divide-sage-200 dark:divide-[#2C2C2E] text-xs">
        {outcomes.map((co) => (
          <li key={co.id} className="py-1.5 flex items-start gap-2">
            <span className="font-mono font-bold text-sage-800 dark:text-white w-12 shrink-0">{co.display_code}</span>
            <span className="text-sage-700 dark:text-sage-200 flex-1">{co.description}</span>
            {co.cognitive_level && <span className="text-[10px] text-sage-500 shrink-0">{co.cognitive_level}</span>}
          </li>
        ))}
      </ul>
    )}
  ...
  return (
    <div className="space-y-1.5" data-testid="program-outcome-list">
      <div className="flex items-center justify-between">
        <h4 className="text-[10px] uppercase tracking-wider font-semibold text-sage-500">Program Outcomes ({outcomes.length})</h4>
      ...
      {outcomes.length === 0 && !adding && <p className="text-xs text-sage-500 italic">No program outcomes configured. Add the outcomes your institution defines.</p>}
      <ul className="divide-y divide-sage-200 dark:divide-[#2C2C2E] text-xs">
        {outcomes.map((po) => (
          <li key={po.id} className="py-1.5 flex items-start gap-2" data-testid="po-row">
            <span className="font-mono font-bold text-sage-800 dark:text-white w-12 shrink-0">{po.code}</span>
            <span className="text-sage-700 dark:text-sage-200 flex-1">{po.title}{po.description ? <span className="text-sage-500"> — {po.description}</span> : null}</span>
            {canEdit && onDelete && (
              <button type="button" onClick={() => onDelete(po.id)} className="text-sage-500 hover:text-red-600" aria-label={`Remove ${po.code}`}><Trash2 className="w-3.5 h-3.5" /></button>
            )}
          </li>
        ))}
      </ul>
      {adding && (
        <div className="p-3 rounded-lg border border-sage-200 dark:border-[#3A3A3C] space-y-2" data-testid="po-form">
