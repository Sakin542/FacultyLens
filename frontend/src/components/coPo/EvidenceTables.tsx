...
export const CoCoverageTable: React.FC<{ rows: CoCoverage[] }> = ({ rows }) => (
  <div className="overflow-x-auto rounded-lg border border-sage-200 dark:border-[#3A3A3C]" data-testid="co-coverage-table">
      ...
      <tbody className="divide-y divide-sage-200 dark:divide-[#2C2C2E]">
        {rows.map((co) => (
          <tr key={co.learning_outcome_id} data-testid="co-coverage-row">
            <td className="px-3 py-2"><span className="font-mono font-semibold text-sage-800 dark:text-white">{co.display_code}</span> <span className="text-sage-500">{co.description}</span></td>
            ...
            <td className="px-3 py-2 text-right font-mono text-sage-800 dark:text-white">{pct(co.coverage_percent)}</td>
            <td className="px-3 py-2 text-right font-mono">{co.po_mapping_count}</td>
            <td className="px-3 py-2 text-sage-500">{co.coverage_status.replace(/_/g, ' ').toLowerCase()}</td>
          </tr>
        ))}
      </tbody>
    </table>
...
export const CoPerformanceMatrix: React.FC<{ rows: CoCoverage[] }> = ({ rows }) => (
  <div className="space-y-2" data-testid="co-performance-matrix">
    <h4 className="text-[10px] uppercase tracking-wider font-semibold text-sage-500">CO Coverage &amp; Performance</h4>
        ...
        <tbody className="divide-y divide-sage-200 dark:divide-[#2C2C2E]">
          {rows.map((co) => (
            <tr key={co.learning_outcome_id} data-testid="co-performance-row">
              <td className="px-3 py-2 font-mono font-semibold text-sage-800 dark:text-white" title={co.description}>{co.display_code}</td>
              <td className="px-3 py-2 text-right font-mono">{pct(co.coverage_percent)}</td>
              <td className="px-3 py-2 text-right font-mono text-sage-800 dark:text-white">{pct(co.performance_percent)}</td>
              <td className="px-3 py-2 text-right font-mono text-sage-500">{gap(co.performance_gap)}</td>
              <td className="px-3 py-2 text-right font-mono text-sage-500">{co.response_count || '—'}</td>
...
export const PoEvidenceMatrix: React.FC<{ rows: PoEvidence[] }> = ({ rows }) => (
  <div className="space-y-2" data-testid="po-evidence-matrix">
    <h4 className="text-[10px] uppercase tracking-wider font-semibold text-sage-500">PO Evidence</h4>
    {rows.length === 0 ? (
      <p className="text-xs text-sage-500 italic">No program outcomes configured for this course's program.</p>
    ) : (
      <div className="overflow-x-auto rounded-lg border border-sage-200 dark:border-[#3A3A3C]">
        <table className="w-full text-xs">
          <thead className="bg-sage-100 dark:bg-[#2C2C2E] text-[10px] uppercase tracking-wider text-sage-500">
          ...
          <tbody className="divide-y divide-sage-200 dark:divide-[#2C2C2E]">
            {rows.map((po) => (
              <tr key={po.program_outcome_id} data-testid="po-evidence-row">
                <td className="px-3 py-2"><span className="font-mono font-semibold text-sage-800 dark:text-white">{po.code}</span> <span className="text-sage-500">{po.title}</span></td>
                <td className="px-3 py-2 text-sage-500">{po.mapped_cos.length ? po.mapped_cos.map((c) => `${c.code} (${c.level})`).join(', ') : '—'}</td>
                ...
                <td className="px-3 py-2 text-right font-mono text-sage-500">{pct(po.contribution_percent)}</td>
                <td className="px-3 py-2 text-right font-mono text-sage-800 dark:text-white">{pct(po.student_performance_percent)}</td>
    ...
    <p className="text-[11px] text-sage-500">Contribution = Σ(CO coverage × level weight 1/3, 2/3, 1). "Evidence available" means evidence exists for review — not that a PO is achieved.</p>
  </div>
);
...
export const PoCoverageTable: React.FC<{ rows: PoEvidence[] }> = ({ rows }) => (
  <div className="overflow-x-auto rounded-lg border border-sage-200 dark:border-[#3A3A3C]" data-testid="po-coverage-table">
    <table className="w-full text-xs">
      <thead className="bg-sage-100 dark:bg-[#2C2C2E] text-[10px] uppercase tracking-wider text-sage-500">
      ...
      <tbody className="divide-y divide-sage-200 dark:divide-[#2C2C2E]">
        {rows.map((po) => (
          <tr key={po.program_outcome_id}>
            <td className="px-3 py-2 font-mono font-semibold text-sage-800 dark:text-white">{po.code}</td>
            <td className="px-3 py-2 text-right font-mono">{po.mapped_co_count}</td>
            <td className="px-3 py-2 text-right font-mono">{pct(po.assessment_evidence_percent)}</td>
            <td className="px-3 py-2 text-sage-500">{po.evidence_status.replace(/_/g, ' ').toLowerCase()}</td>
          </tr>
        ))}
      </tbody>
    </table>
