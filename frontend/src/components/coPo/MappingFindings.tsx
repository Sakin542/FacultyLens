...
export const MappingFindingCard: React.FC<{ finding: MappingFinding }> = ({ finding }) => (
  <li className="p-3 rounded-lg border border-sage-200 dark:border-[#3A3A3C] bg-white dark:bg-[#1C1C1E] space-y-1.5" data-testid="mapping-finding">
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0">
        <p className="text-xs font-semibold text-sage-800 dark:text-white">{finding.title}</p>
        <p className="text-[11px] text-sage-500">{finding.description}</p>
      </div>
    ...
    {finding.recommendation && (
      <p className="text-xs text-sage-700 dark:text-sage-200 pl-2 border-l-2 border-sage-200 dark:border-[#3A3A3C]" data-testid="finding-recommendation">
        {finding.recommendation}
      </p>
    )}
    <p className="text-[10px] text-sage-500 font-mono">{finding.type.replace(/_/g, ' ').toLowerCase()}{finding.category ? ` · ${finding.category.replace(/_/g, ' ')}` : ''}{finding.priority ? ` · priority ${finding.priority}` : ''}</p>
  </li>
);
...
export const MappingFindings: React.FC<{ findings: MappingFinding[] }> = ({ findings }) => (
  <div className="space-y-2" data-testid="mapping-findings">
    <h4 className="text-[10px] uppercase tracking-wider font-semibold text-sage-500">Validation Findings ({findings.length})</h4>
    {findings.length === 0 ? (
      <p className="text-xs text-sage-500 italic">No mapping review signals were generated.</p>
    ) : (
    ...
    <div className="space-y-3" data-testid="mapping-validation-summary">
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        {tiles.map(([label, value]) => (
          <div key={label} className="p-3 rounded-xl bg-sage-100 dark:bg-[#2C2C2E] text-center" data-testid="mapping-tile">
        ...
        <div className="space-y-1.5" data-testid="mapping-bars">
          {bars.filter(([, v]) => v !== undefined).map(([label, v]) => (
            <div key={label} className="flex items-center gap-3 text-xs">
              <span className="w-32 text-sage-500">{label}</span>
              <div className="flex-1 h-2 rounded-full bg-sage-200 dark:bg-[#3A3A3C] overflow-hidden"><div className="h-full bg-sage-700 dark:bg-white rounded-full" style={{ width: `${Math.min(v as number, 100)}%` }} /></div>
              <span className="w-12 text-right font-mono text-sage-800 dark:text-white">{v}%</span>
            </div>
          ))}
        </div>
      )}
      {summary.validation_checks && (
        <ul className="space-y-1 text-xs" data-testid="validation-checks">
          {summary.validation_checks.map((c) => (
            <li key={c.label} className="flex items-center gap-2 text-sage-700 dark:text-sage-200">
      ...
      <p className="text-[11px] text-sage-500 flex items-center gap-1"><Info className="w-3 h-3" /> Density = non-zero cells ÷ (COs × POs). Coverage = marks attributed to a CO ÷ total assessment marks.</p>
    </div>
  );
};
