...
export const TopicPerformance: React.FC<TopicPerformanceProps> = ({ topics, expected, hasQuestions }) => (
  <div className="space-y-2" data-testid="topic-performance">
    <h4 className="text-[10px] uppercase tracking-wider font-semibold text-[#737373]">Topic Performance</h4>
    {topics.length === 0 ? (
        ...
        {topics.map((t) => (
          <li key={t.id} className="space-y-1" data-testid="topic-row">
            <div className="flex items-center justify-between gap-2 text-xs">
              <span className="font-medium text-[#111111] dark:text-white truncate">{t.topic}</span>
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-[10px] text-[#737373]">{t.question_count} q · {t.response_count} responses</span>
                <span className="font-mono text-[#111111] dark:text-white">{formatPct(t.average_percentage)}</span>
                <span className="text-[10px] text-[#737373] w-14 text-right">{formatGap(t.performance_gap)}</span>
