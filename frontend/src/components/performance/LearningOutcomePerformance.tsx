...
export const LearningOutcomePerformance: React.FC<LearningOutcomePerformanceProps> = ({ outcomes, questions, expected }) => {
  ...
  return (
    <div className="space-y-2" data-testid="lo-performance">
      <h4 className="text-[10px] uppercase tracking-wider font-semibold text-sage-500">Learning Outcome Performance</h4>
      {outcomes.length === 0 ? (
        <p className="text-xs text-sage-500 italic">This course has no learning outcomes yet.</p>
      ) : (
            ...
            <li key={lo.id} className="space-y-1" data-testid="lo-row">
              <div className="flex items-start justify-between gap-2 text-xs">
                <div className="min-w-0">
                  <span className="font-bold font-mono text-sage-800 dark:text-white">{lo.lo_code}</span>
                  {lo.lo_description && <span className="text-sage-500 ml-2">{lo.lo_description}</span>}
                  <div className="text-[10px] text-sage-500 mt-0.5">
                    {lo.question_count === 0 ? 'No questions mapped to this outcome in this assessment.' : (
                      <>Questions: {lo.question_ids.map((qid, i) => (
                        <a key={qid} href={`#question-${qid}`} className="underline hover:text-sage-800 dark:hover:text-white" data-testid="lo-question-link">Q{numberOf(qid)}{i < lo.question_ids.length - 1 ? ', ' : ''}</a>
                      ))} · {lo.response_count} responses</>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="font-mono text-sage-800 dark:text-white">{formatPct(lo.average_percentage)}</span>
                  <span className="text-[10px] text-sage-500 w-14 text-right">{formatGap(lo.performance_gap)}</span>
