...
const BAR: Record<PerformanceStatus, string> = {
  ...
  ON_TARGET: 'bg-sage-700 dark:bg-white',
  MINOR_GAP: 'bg-sage-300',
  MODERATE_GAP: 'bg-amber-500',
  HIGH_GAP: 'bg-red-400',
  INSUFFICIENT_DATA: 'bg-sage-200 dark:bg-[#3A3A3C]',
};
...
export const PerformanceBar: React.FC<PerformanceBarProps> = ({ value, status, expected }) => (
  <div className="relative w-full h-2.5 rounded-full bg-sage-200 dark:bg-[#3A3A3C] overflow-hidden" aria-hidden="true" data-testid="performance-bar">
    <div className="absolute top-0 bottom-0 w-0.5 bg-sage-400 z-10" style={{ left: `${Math.min(Math.max(expected, 0), 100)}%` }} />
