...
export const QuestionPerformanceCard: React.FC<{ q: QuestionPerformance; expected: number; defaultOpen?: boolean }> = ({ q, expected, defaultOpen = false }) => {
  ...
  return (
    <li className="rounded-lg border border-sage-200 dark:border-[#3A3A3C] bg-white dark:bg-[#1C1C1E]" data-testid="question-performance-card">
      <button type="button" onClick={() => setOpen((v) => !v)} className="w-full p-3 text-left space-y-2" aria-expanded={open}>
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <span className="text-xs font-bold font-mono text-sage-800 dark:text-white">Q{q.question_number ?? q.question_id}</span>
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-xs font-mono text-sage-700 dark:text-sage-200" data-testid="question-average">{formatPct(q.average_percentage)}</span>
            <span className="text-[10px] text-sage-500">{q.response_count} / {q.submission_count} finalized</span>
            <PerformanceGapBadge status={q.performance_status} />
            {open ? <ChevronUp className="w-4 h-4 text-sage-500" /> : <ChevronDown className="w-4 h-4 text-sage-500" />}
          </div>
        </div>
        <PerformanceBar value={q.average_percentage} status={q.performance_status} expected={expected} />
        {q.question_text_excerpt && <p className="text-[11px] text-sage-500 truncate">{q.question_text_excerpt}</p>}
      </button>
      {open && (
        <div className="px-3 pb-3 pt-2 border-t border-sage-200 dark:border-[#2C2C2E] space-y-3 text-xs" data-testid="question-performance-details">
          <dl className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            {([
              ...
              <div key={k} className="p-2 rounded-lg bg-sage-100 dark:bg-[#2C2C2E]">
                <dt className="text-[10px] uppercase tracking-wider text-sage-500">{k}</dt>
                <dd className="font-mono text-sage-800 dark:text-white">{v}</dd>
              </div>
            ))}
          </dl>
          {(q.ai_suggested_average_percentage !== null || q.rubric_alignment_average !== null) && (
            <div className="grid grid-cols-2 gap-2" data-testid="question-context">
              {q.ai_suggested_average_percentage !== null && (
                <div className="p-2 rounded-lg border border-sage-200 dark:border-[#3A3A3C]">
                  <span className="block text-[10px] uppercase tracking-wider text-sage-500">AI suggested (supporting only)</span>
                  <span className="font-mono">{formatPct(q.ai_suggested_average_percentage)}</span>
                </div>
              )}
              {q.rubric_alignment_average !== null && (
                <div className="p-2 rounded-lg border border-sage-200 dark:border-[#3A3A3C]">
                  <span className="block text-[10px] uppercase tracking-wider text-sage-500">Rubric alignment (avg)</span>
          ...
          {q.review_signals.length > 0 && (
            <div>
              <span className="block text-[10px] uppercase tracking-wider text-sage-500 mb-1">Review signals</span>
              <ul className="list-disc pl-4 space-y-0.5 text-sage-700 dark:text-sage-200" data-testid="review-signals">
