...
export const QuestionPerformanceTable: React.FC<QuestionPerformanceTableProps> = ({ questions, expected }) => {
  ...
  if (questions.length === 0) {
    return <p className="text-xs text-[#737373] italic">This assessment has no questions.</p>;
  }
  return (
    <div className="space-y-2" data-testid="question-performance">
      <div className="flex items-center justify-between">
        <h4 className="text-[10px] uppercase tracking-wider font-semibold text-[#737373]">Question Performance</h4>
        <div className="flex items-center gap-1">
          <button type="button" onClick={() => setView('table')} className={`p-1.5 rounded-md ${view === 'table' ? 'bg-[#111111] text-white dark:bg-white dark:text-[#111111]' : 'text-[#737373]'}`} aria-label="Table view"><Table className="w-3.5 h-3.5" /></button>
          <button type="button" onClick={() => setView('cards')} className={`p-1.5 rounded-md ${view === 'cards' ? 'bg-[#111111] text-white dark:bg-white dark:text-[#111111]' : 'text-[#737373]'}`} aria-label="Detail view" data-testid="detail-view"><LayoutList className="w-3.5 h-3.5" /></button>
        </div>
      </div>
      {view === 'table' ? (
            ...
            <tbody className="divide-y divide-[#E5E5E5] dark:divide-[#2C2C2E]">
              {questions.map((q) => (
                <tr key={q.id} data-testid="question-row">
                  <td className="px-3 py-2 font-mono font-semibold text-[#111111] dark:text-white">Q{q.question_number ?? q.question_id}</td>
                  <td className="px-3 py-2 text-right font-mono text-[#111111] dark:text-white">{formatPct(q.average_percentage)}</td>
                  <td className="px-3 py-2 text-right font-mono text-[#737373]">{fmt(q.maximum_marks)}</td>
                  <td className="px-3 py-2 text-right font-mono text-[#737373]">{q.response_count} / {q.submission_count}</td>
                  <td className="px-3 py-2 text-right font-mono text-[#737373]">{formatGap(q.performance_gap)}</td>
                  <td className="px-3 py-2 text-[#737373] capitalize">{q.difficulty_level ?? '—'}</td>
