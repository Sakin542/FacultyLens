    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {cards.map((card, idx) => {
        ...
        return (
          <Card
            key={idx}
            className="p-4 bg-white dark:bg-[#1C1C1E] border border-sage-200 dark:border-[#2C2C2E] shadow-sm flex flex-col justify-between space-y-3 hover:border-sage-700 dark:hover:border-white transition-colors"
          >
            <div className="space-y-2">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-sage-800 dark:text-white leading-tight">
                      {card.name}
                    </h3>
                  </div>
                </div>

                <span className="font-mono text-base font-extrabold text-sage-800 dark:text-white">
                  {val !== null ? `${val}%` : '—'}
                </span>
              </div>

              <p className="text-[11px] text-sage-500 leading-relaxed line-clamp-2">
                {card.explanation}
              </p>
            </div>
            ...
            <div className="pt-2 border-t border-sage-200 dark:border-[#2C2C2E]">
              {val !== null ? (
                ...
                <span className="text-[10px] text-sage-500 italic">Analysis pending</span>
              )}
            </div>
          </Card>
        );
