        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm"><caption className="sr-only">Learning outcome alignment details</caption>
            ...
            <tbody>{data.outcomes.map((o) => (
              <tr key={o.learning_outcome_id} className="border-t border-sage-100 dark:border-[#2A2A2A]">
                <td className="py-1 pr-3"><span className="font-medium">{o.code}</span>{o.course_code && <span className="text-xs text-sage-400"> · {o.course_code}</span>}<p className="text-xs text-sage-500 truncate max-w-xs" title={o.description}>{o.description}</p></td>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm"><caption className="sr-only">Program outcome evidence</caption>
          ...
          <tbody>{data.program_outcomes.map((p) => (
            <tr key={p.code} className="border-t border-sage-100 dark:border-[#2A2A2A]">
              <td className="py-1 pr-3"><span className="font-medium">{p.code}</span><p className="text-xs text-sage-500 truncate max-w-xs" title={p.title}>{p.title}</p></td>
        ...
        {data.unmapped_questions ? <p className="text-xs text-sage-500 mt-1">{data.unmapped_questions} question(s) have no confirmed CO mapping.</p> : null}
      </div>
    )}
  </Section>
);
  ...
  <Section testId="student-performance" title="Student Performance" explanation={data.explanation} actions={data.available ? <StatusBadge status={data.status} /> : undefined}>
    {restricted ? <SectionEmpty title="Restricted for your role" description="Student-level performance is available to course owners and editors only." /> : !data.available ? (
      ...
      <>
        ...
        <p className="text-xs text-sage-500">Benchmark {data.benchmark_percent}% · gap {data.gap === null ? 'N/A' : `${data.gap}%`} · status {humanize(data.status)}{data.status === 'INSUFFICIENT_DATA' ? ' (fewer responses than the configured minimum — not classified as a gap)' : ''}.</p>
    ...
    {data.analyzed_assessments === 0 ? <SectionEmpty title="No performance analysis yet" description="Learning gaps appear after a student performance analysis has been run." /> : (
      <>
        <dl className="grid grid-cols-3 md:grid-cols-6 gap-2 text-sm">
          {GAP_ORDER.map((s) => <div key={s} className="rounded-md border border-sage-100 dark:border-[#2A2A2A] px-2 py-1"><dt className="text-xs"><StatusBadge status={s} /></dt><dd className="font-semibold tabular-nums">{data.counts[s] ?? 0}</dd></div>)}
        </dl>
        {data.top_gaps.length > 0 && (
          <ol className="space-y-1 text-sm" aria-label="Top learning gaps">
            {data.top_gaps.map((g, i) => (
              <li key={`${g.code}-${g.assessment_id}`} className="flex flex-wrap items-baseline gap-x-2 border-t border-sage-100 dark:border-[#2A2A2A] pt-1">
                <span className="font-medium">{i + 1}. {g.code}</span><span className="text-xs text-sage-500">{g.description}</span>
      ...
      {sorted.length === 0 ? <SectionEmpty title="No question-level performance" description="Question performance appears after a student performance analysis has been run." /> : (
        <div className="overflow-x-auto">
          <table className="w-full text-sm"><caption className="sr-only">Question-level performance from finalized grades</caption>
            ...
            <tbody>{sorted.map((q) => (
              <tr key={`${q.assessment_id}-${q.question_number}`} className="border-t border-sage-100 dark:border-[#2A2A2A]">
                <td className="py-1 pr-3"><span className="font-medium">Q{q.question_number}</span><span className="text-xs text-sage-400"> · {q.assessment_title}</span>{q.excerpt && <p className="text-xs text-sage-500 truncate max-w-xs" title={q.excerpt}>{q.excerpt}</p>}</td>
    ...
    {rows.length === 0 ? <SectionEmpty title="No topic-level performance" description="Topics appear after question analysis and a student performance analysis have been run." /> : (
      <div className="overflow-x-auto">
        <table className="w-full text-sm"><caption className="sr-only">Topic performance</caption>
          <thead><tr className="text-left text-xs uppercase text-sage-500"><th className="py-1 pr-3">Topic</th><th className="py-1 pr-3 text-right">Questions</th><th className="py-1 pr-3 text-right">Average</th><th className="py-1 pr-3 text-right">Responses</th><th className="py-1 pr-3 text-right">Gap</th><th className="py-1">Status</th></tr></thead>
          <tbody>{rows.map((t) => (
            <tr key={t.topic} className={`border-t border-sage-100 dark:border-[#2A2A2A] ${selected === t.topic ? 'bg-[#FAFAF8] dark:bg-[#1A1A1A]' : ''}`}>
    ...
    <Section testId="assessment-comparison" title="Compare Assessments" subtitle="Select two or more assessments in scope." actions={<div className="flex gap-2"><Button size="sm" onClick={onCompare} disabled={selected.length < 2 || loading}>{loading ? 'Comparing…' : 'Compare'}</Button>{comparison && <Button size="sm" variant="ghost" onClick={onClear}>Clear</Button>}</div>}>
      ...
      {comparison && (
        <div className="overflow-x-auto">
          ...
          <p className="text-xs text-sage-500 mt-1">{comparison.note}</p>
        </div>
      )}
    </Section>
  );
...
export const CourseHistoryTable: React.FC<{ history: CourseHistory | null; loading?: boolean; error?: string | null; onRetry?: () => void }> = ({ history, loading, error, onRetry }) => (
  <Section testId="course-history" title="Historical Course Analytics" subtitle={history ? `${history.course_code} — ${history.course_name} across ${history.course_ids.length} term(s)` : 'Select a course filter to see term-over-term trends.'}>
    {loading && <p className="text-sm text-sage-500">Loading history…</p>}
    ...
    {rows.length === 0 ? <SectionEmpty title="No assessments in scope" description="Adjust the filters or create an assessment." /> : (
      <div className="overflow-x-auto">
        <table className="w-full text-sm"><caption className="sr-only">Assessments with headline analytics</caption>
          ...
          <tbody>{rows.map((a) => (
            <tr key={a.assessment_id} className="border-t border-sage-100 dark:border-[#2A2A2A]">
              <td className="py-1 pr-3"><Link to={`/assessments/${a.assessment_id}`} className="font-medium underline underline-offset-2">{a.title}</Link><span className="text-xs text-sage-400"> · {a.course?.code} · {humanize(a.type)}</span></td>
              <td className="py-1 pr-3 text-xs">{a.date ?? 'No date'}</td><td className="py-1 pr-3 text-right tabular-nums">{a.questions}</td>
              <td className="py-1 pr-3 text-right tabular-nums">{a.quality_score === null ? <span className="text-xs text-sage-400">Not analyzed</span> : <>{fmtNum(a.quality_score)} <Badge variant={a.quality_rating ? (a.quality_rating === 'EXCELLENT' || a.quality_rating === 'GOOD' ? 'Good' : a.quality_rating === 'FAIR' ? 'Attention' : 'Critical') : 'neutral'} size="sm">{humanize(a.quality_rating)}</Badge></>}</td>
              <td className="py-1 pr-3 text-right tabular-nums">{a.performance_percentage === null ? <span className="text-xs text-sage-400">N/A</span> : fmtPct(a.performance_percentage)}</td>
