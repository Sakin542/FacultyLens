...
export const NotificationEmptyState: React.FC<NotificationEmptyStateProps> = ({
  ...
  <div data-testid="notification-empty" className={cn('flex flex-col items-center text-center', compact ? 'px-4 py-8' : 'px-6 py-14', className)}>
    <span className={cn('rounded-full bg-sage-100 border border-sage-200 text-sage-500 flex items-center justify-center mb-3', compact ? 'w-9 h-9' : 'w-12 h-12')}>
      <BellOff className={compact ? 'w-4 h-4' : 'w-5 h-5'} aria-hidden="true" strokeWidth={1.8} />
    </span>
    <p className={cn('font-semibold text-sage-800', compact ? 'text-sm' : 'text-base')}>{title}</p>
    <p className={cn('text-sage-500 mt-1 max-w-xs', compact ? 'text-xs' : 'text-sm')}>{description}</p>
  </div>
);
