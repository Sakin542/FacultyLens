...
export const NotificationPreferences: React.FC<{ className?: string }> = ({ className }) => {
  ...
  return (
    <form className={cn('space-y-4', className)} onSubmit={(e) => { e.preventDefault(); void save(); }} noValidate data-testid="notification-preferences">
      <p className="text-sm text-sage-500">Choose which in-app notifications you receive. Security and system notices are always delivered. Turning a notification off never changes the underlying academic workflow — analyses, reports and reviews continue exactly as before.</p>
            ...
            <div className="flex items-start justify-between gap-3 px-4 py-3 bg-sage-100 border-b border-sage-200">
              ...
              {!mandatory && (
                <div className="flex items-center gap-1 shrink-0 text-xs">
                  <button type="button" onClick={() => setCategory(category, true)} className="rounded-md px-2 py-1 text-sage-700 hover:bg-white focus-visible:outline-2 focus-visible:outline-sage-700" aria-label={`Enable all ${CATEGORY_LABELS[category]} notifications`}>All</button>
                  <span aria-hidden="true" className="text-sage-200">|</span>
                  <button type="button" onClick={() => setCategory(category, false)} className="rounded-md px-2 py-1 text-sage-500 hover:bg-white focus-visible:outline-2 focus-visible:outline-sage-700" aria-label={`Disable all ${CATEGORY_LABELS[category]} notifications`}>None</button>
              ...
              {rows.map((p) => {
                ...
                return (
                  <li key={p.notification_type} className="flex items-center justify-between gap-3 px-4 py-2.5">
                    <label htmlFor={inputId} className="text-sm text-sage-800 cursor-pointer flex-1">{p.label}</label>
                    <span className="flex items-center gap-2">
                      {p.mandatory && <span className="text-[10px] uppercase tracking-wide text-sage-500">Always on</span>}
                      <input
                        id={inputId}
                        type="checkbox"
                        role="switch"
                        ...
                        className="h-4 w-4 rounded border-sage-200 text-sage-700 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sage-700 disabled:opacity-60"
                      />
                    </span>
                  </li>
                );
